...
static size_t **get_list(size_t size) {
    if (size <= 0) {
        ...
        return array_head + size - 1;
    } else if (size <= 4096) {
        return array_head + ilog2(size - 1) + 58;
    } else {
...
static void place(void *bp, size_t alloc_block_size) {
    ...
    if (size - alloc_block_size >= 3 * DSIZE) {
...
void *malloc (size_t size) {
    ...
    //bp = extend_heap(MAX(total_size, (CHUNKSIZE / 63 + 7) / 8 * 8));
    bp = extend_heap(total_size);
