...
/*
 * get_list - return a relative pointer to the list head by size
 */
static unsigned **get_list(size_t size) {
    if (size <= 3) {
        return NULL;
...
static void insert_to_list(void *bp) {
    ...
    *(p + 1) = (unsigned)*list_p;
    *p = (unsigned)NULL;
    *list_p = (unsigned *)((size_t)p - base); /* pred */
    if (*(p + 1) != (unsigned)NULL) { /* succ */
        *(unsigned *)(base + (size_t)*(p + 1)) = (unsigned)((size_t)p - base);
    }
}
...
static void delete_from_list(void *bp) {
    ...
    p = (unsigned *)bp;
    pred = (unsigned *)(base + (size_t)*p);
    succ = (unsigned *)(base + (size_t)*(p + 1));
    if (*p != (unsigned)NULL) { /* have a predecessor block */
        *(pred + 1) = *(p + 1);
    } else { /* first block in the free list */
        list_p = get_list(block_size(bp));
        *list_p = (unsigned *)((size_t)succ - base);
    }
    if (*(p + 1) != (unsigned)NULL) { /* have a successor block */
        *succ = *p;
    }
    *p = (unsigned)NULL;
    *(p + 1) = (unsigned)NULL;
...
void mm_checkheap(int verbose){
    ...
    for (i = 0; i < ARRAYSIZE; i++) {
        p = *list_p;
        while (p != NULL) {
            if (!(mem_heap_lo() <= (void *)p && (void *)p <= mem_heap_hi())) {
                printf("Invariant Error: pointer is exceeded\n");
            }
            if (*p) {
                if (*((unsigned *)(base + (size_t)*p) + 1) != (size_t)p - base) {
                    printf("Invariant Error: inconsistent pointer to pred\n");
                }
            }
            if (*(p + 1)) {
                if (*(unsigned *)(base + (size_t)*(p + 1)) != (size_t)p - base) {
                    printf("Invariant Error: inconsistent pointer to succ\n");
                }
            }
            if (get_list(block_size((void *)p)) != list_p) {
                printf("Invariant Error: block size doesn't match list\n");
            }
            p = (unsigned *)(base + (size_t)*p);
        }
        list_p++;
    }
    //print_heap();
    ...
    if (DEBUG_CHECKHEAP) {
        ...
        while (!size == 0) {
            ...
            if (alloc == 0) { // free block
                ...
                printf("succ: 0x%lx -> 0x%x\n", (long)(bp + WSIZE), *((unsigned *)(bp + WSIZE)));
