...
static unsigned *get_list(size_t size) {
    if (size <= 3) {
        ...
        return array_head + (ilog2(size - 1) + 81);
    } else {
...
void *malloc (size_t size) {
    ...
    while (list_p != array_tail) {
        ...
        while (bp != NULL) {
            if (block_size((void *)r2a((size_t)bp)) >= total_size) {
                ...
                dbg_printf("want to return 0x%lx from malloc(%d) after find a block big enough \n", (long)bp, (int)size);
                //print_heap();
    ...
    bp = extend_heap(MAX(total_size, 80));
    //bp = extend_heap(total_size);
    ...
    dbg_printf("want to return 0x%lx from malloc(%d) and can't find big enough block\n", (long)bp, (int)size);
    //print_heap();
    return (void *)bp;
}
...
static void print_heap() {
    ...
    if (DEBUG_CHECKHEAP) {
        ...
        for (; i < ARRAYSIZE; i++) {
            printf("0x%lx:  [%d, %d] -> %x\n", (long)list_p, (1 << (i - 81)) + 1, 1 << (i - 80), (unsigned)*list_p);
            list_p++;
        }
        printf("----- blocks -----\n");
