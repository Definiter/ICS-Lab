...
void builtin_bg(int id, int is_pid);
void builtin_fg(int id, int is_pid);
...
/* System function wrappers (via csapp.c) */
pid_t Fork();
...
eval(char *cmdline) 
{
    ...
    pid_t pid; /* child pid */
    sigset_t mask, old_mask; /* signal mask & saved old mask */ 
    ...
    if (tok.builtins == BUILTIN_NONE) {  /* executable file */
        ...
        /* child process */
        if ((pid = Fork()) == 0) { 
            Sigprocmask(SIG_SETMASK, &old_mask, NULL); /* since child process may 
                                                      become new parent process */
            if (tok.infile) { /* input redirection */
                fd_in = Open(tok.infile, O_RDONLY, 0);
                Dup2(fd_in, 0);
            }
            if (tok.outfile) { /* output redirection */
                fd_out = Open(tok.outfile, O_WRONLY | O_CREAT | O_TRUNC, DEF_MODE);
                Dup2(fd_out, 1);
            } 

            setpgid(0, 0); /* assign child process group id */
        ...
        if (!bg) { /* run in foreground */
            ...
            while (fgpid(job_list)) { /* wait foreground process to be terminated */
                pause();
            }
        ...
        if (tok.builtins == BUILTIN_QUIT) {
            ...
            int id;
            if (tok.argv[1][0] == '%') { // JID
                id = strtol(tok.argv[1] + 1, endptr, 10);
                if (endptr == NULL) {
                ...
                id = strtol(tok.argv[1], endptr, 10);
                if (endptr == NULL) {
                    if (tok.builtins == BUILTIN_BG){
                        builtin_bg(id, 1);
                    } else {
...
/*
 * builtin_bg - built-in command bg
 */
void builtin_bg(int id, int is_pid) {
    int pid;
    struct job_t *job;
    ...
    if (job->state == ST) {
        ...
        app_error("the job is not stopped");
    }
}
/*
 * builtin_fg - built-in command fg
 */
void builtin_fg(int id, int is_pid) {
    int pid;
    struct job_t *job;
    ...
    while (fgpid(job_list)) {
        pause(); /* wait foreground process to be terminated */
    }
}
/* 
...
sigchld_handler(int sig) 
{
    ...
    struct job_t *job;
    int status;
    /* reaps all zombie children, and modify stopped children job info */
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        job = getjobpid(job_list, pid);
        if (WIFSTOPPED(status)) { // child process stopped
            job->state = ST;
...
sigint_handler(int sig) 
{
    ...
    if (pid) {
        Kill(-pid, SIGINT); /* kill pid group */
    } 
    return;
}
...
sigtstp_handler(int sig) 
{
    pid_t pid = fgpid(job_list);
    if (pid) {
        Kill(-pid, SIGTSTP); /* stop pid group */
    } 
    return;
}
