# Hao Wang 1300012703 #
#
# 1. Add pipe line version of iaddl instruction, by referring to
# computations for iaddl (see below) and the HCL implementation 
# of opl instruction. It's motivation is to combine irmovl & addl 
# to iaddl -- one instruction.
# 
###########################################
#
# computations for iaddl 
#
...
int d_valA = [
	...
	d_srcA == e_dstE : e_valE;    # Forward valE from execute, consider cmov
	d_srcA == M_dstM : m_valM;    # Forward valM from memory, m_valM 
                                  # just generate in M stage
...
int e_dstE = [
	E_icode == IRRMOVL && !e_Cnd : RNONE; ## consider cmov
	1 : E_dstE;
];
